function out = growth_par_elmloop(damage_params, days,k_sigma_f, k_sigma_m, name, length, n_dt, print_step, kc, P_a, r_h, H_h, nu_e0, nu_f0, nu_m0, phi0, G_h, G_e, G_m, Sa, La_M, La_0, sigma_f0, sigma_m0, n_elm, kq_c, kq_m, age_max, op_time)

timer.t1 = 0; timer.t2 = 0; timer.t3 = 0; timer.t4 = 0; timer.t5 = 0; timer.t6 = 0; timer.t7 = 0; timer.t8 = 0;
...
type = 2;  %type of the shape function 1=linear or 2=quadratic
...
file_name = sprintf('%s_middle_point.dat',name);
if WRITE_FILE
    fid0=fopen(file_name,'w');
end

shapefiler_name = [name '_shaper.dat'];
shapefilez_name = [name '_shapez.dat'];
if WRITE_FILE
    shapefiler_fid=fopen(shapefiler_name,'w');
    shapefilez_fid=fopen(shapefilez_name,'w');
end
...
switch(type) % 1=linear, 2=quadratic
    case 1
        n_node = n_elm+1;
        sh = l_elm;
        npe = 2;
...
TD_L1_sliced=zeros(n_elm,n_Gauss_pt,n_time+1);    % lambda_1(points,time)
TD_L2_sliced=zeros(n_elm,n_Gauss_pt,n_time+1);    % lambda_2(points, time)
...
for i=2:num_DL
    t= dt*(i-1);
    DQ_c(i) = DQ_c(i-1) + 0.5*dt*(q_i(0, t-dt, kq_c, kq_m)+q_i(0,t, kq_c, kq_m));
    DQ_m(i) = DQ_m(i-1) + 0.5*dt*(q_i(1, t-dt, kq_c, kq_m)+q_i(1,t, kq_c, kq_m));
end
...
%--------------------------------------------------
timer.t1 = timer.t1 + (cputime - cput1);
...
for nt=1:n_time
    ...
    current_t=dt*nt;
    P=P_AAA(current_t, op_time, P_a);
    
    message = sprintf('%3.1f days, pressure %3.4f mmHg, k_stent=%f \n', current_t, P/133.322668,k_stent);
    %     disp(message);
    timer.t2 = timer.t2 + (cputime - cput2);
    ...
    for more_it=1:max_more_it   % interation for mass
        ...
        while ( itnum <= max_itr & itr_error > tol)
            ...
            if (current_t >= op_time) && (k_count<= 20)
                k_count=k_count+1;
                k_stent = k_stent+1;
            end
            ...
            % count=0; % this is function-ized
            
            glk_segments = cell(elm,1);
            glf_segments = cell(elm,1);
            
            timer.t3 = timer.t3 + (cputime - cput3);
            cput4 = cputime;
            ...
            parfor elm=1:n_elm
                ...
                TD_L1_part = squeeze(TD_L1_sliced(elm,:,:));
                TD_L2_part = squeeze(TD_L2_sliced(elm,:,:));
                
                glf_addition = zeros(npe*2,1);
                glk_addition = zeros(npe*2,n_node*2);
                
                for k=1:n_Gauss_pt
                    % count=count+1;
                    count = ((elm-1)*n_Gauss_pt) + k;
                    ...
                    for i=1:npe
                        ...
                        shp1d_i0Z = shp1d(index,0,Z, sh, n_elm, type);
                        shp1d_i1Z = shp1d(index,1,Z, sh, n_elm, type);
                        
                        R = R+Data(index,2)*shp1d_i0Z;
                        dR = dR+Data(index,2)*shp1d_i1Z;
                        
                        z = z +x_np(index*2)*shp1d_i0Z;
                        r = r +x_np(index*2-1)*shp1d_i0Z;
                        dz =dz+x_np(index*2)*shp1d_i1Z;
                        dr = dr+x_np(index*2-1)*shp1d_i1Z;
                    ...
                    if nt+1 <= num_DL  %If there still remains initial mass
                        ...
                        Mf_r=Mf0*DQ_c(nt+1)*damage(Z, damage_params);
                        Mt = Mf_r(1)+Mf_r(2)+Mf_r(3)+Mf_r(4);
                        
                        dwdL_k1= A_11*Mf_r(1)*G_h*dWcdx(Ln_k1, kc);
                        dwdL_k2= A_11*Mf_r(2)*G_h*dWcdx(Ln_k2, kc);
                        dwdL_k3= A_11*Mf_r(3)*G_h*dWcdx(Ln_k3, kc);
                        dwdL_k4= A_11*Mf_r(4)*G_h*dWcdx(Ln_k4, kc);
                        
                        
                        ddwddL_k1=A_11*Mf_r(1)*G_h^2*ddWcddx(Ln_k1, kc);
                        ddwddL_k2=A_11*Mf_r(2)*G_h^2*ddWcddx(Ln_k2, kc);
                        ddwddL_k3=A_11*Mf_r(3)*G_h^2*ddWcddx(Ln_k3, kc);
                        ddwddL_k4=A_11*Mf_r(4)*G_h^2*ddWcddx(Ln_k4, kc);
                        ...
                        Mm = Mm0*DQ_m(nt+1)*damage(Z, damage_params);
                        Mt = Mt+Mm;
                        Ln_m = G_m * L1;
                        dwdL1 = dwdL1+ A_11*Mm*G_m*dWmdx(Ln_m, kc);
                        ddwddL1 = ddwddL1 + A_11*Mm*G_m^2*ddWmddx(Ln_m, kc);
                    ...
                    Me = Me0*damage(Z, damage_params);
                    Mt = Mt+Me;
                    
                    Ln_e1 = G_e(1)*L1;
                    Ln_e2 = G_e(2)*L2;
                    
                    dwdL_e1= A_11*Me*G_e(1)*dWedx(1,Ln_e1, Ln_e2, kc);
                    dwdL_e2= A_11*Me*G_e(2)*dWedx(2,Ln_e1, Ln_e2, kc);
                    ddwddL_e1=A_11*Me*(G_e(1))^2*ddWeddx(1,Ln_e1,Ln_e2, kc);
                    ddwddL_e2=A_11*Me*(G_e(2))^2*ddWeddx(2,Ln_e1,Ln_e2, kc);
                    ddwddL_e1dL_e2 = A_11*Me*G_e(1)*G_e(2)*ddWeddx(3,Ln_e1,Ln_e2, kc);
                    ...
                    for n_tau=n_tau0: (nt+1)
                        ...
                        mc_tau=[TD_mc1(count,n_tau), TD_mc2(count, n_tau), ...
                            TD_mc3(count, n_tau), TD_mc3(count, n_tau)]*q_i(0, current_t-tau, kq_c, kq_m);
                        %                         mm_tau = TD_mm(count, n_tau) * q_i(1, current_t-tau, kq_c, kq_m)...
                        %                             *Rm_exp(0, Z, z_0, current_t, init_dmg_t);
                        mm_tau = TD_mm(count, n_tau) * q_i(1, current_t-tau, kq_c, kq_m)...
                            *damage(Z, damage_params);
                        Mm = Mm+mm_tau*w_dt;
                        ...
                        dwdL_k1 = A_11* mc_tau(1) * (G_h/L1_tau)*dWcdx(Ln_k1, kc);
                        dwdL_k2 = A_11* mc_tau(2) * (G_h/L2_tau)*dWcdx(Ln_k2, kc);
                        dwdL_k3 = A_11* mc_tau(3) * (G_h/L_k3_tau)*dWcdx(Ln_k3, kc);
                        dwdL_k4 = A_11* mc_tau(4) * (G_h/L_k4_tau)*dWcdx(Ln_k4, kc);
                        
                        ddwddL_k1 = A_11*mc_tau(1) * (G_h/L1_tau)^2*ddWcddx(Ln_k1, kc);
                        ddwddL_k2 = A_11*mc_tau(2) * (G_h/L2_tau)^2*ddWcddx(Ln_k2, kc);
                        ddwddL_k3 = A_11*mc_tau(3) * (G_h/L_k3_tau)^2*ddWcddx(Ln_k3, kc);
                        ddwddL_k4 = A_11*mc_tau(4) * (G_h/L_k4_tau)^2*ddWcddx(Ln_k4, kc);
                        ...
                        %Contribution of SM
                        dwdL1 = dwdL1+A_11* mm_tau * (G_m/L1_tau)*dWmdx(Ln_m, kc)*w_dt;
                        ddwddL1 = ddwddL1+A_11*mm_tau * (G_m/L1_tau)^2*ddWmddx(Ln_m, kc)*w_dt;
                    end
                    ...
                    for i=1:npe
                        ii_orig = (elm-1)*(npe-1)+i;
                        ii = i;
                        ii_1=ii*2-1;
                        ii_2=ii*2;
                        
                        shp1d_ii0Z = shp1d(ii_orig,0,Z, sh, n_elm, type);
                        shp1d_ii1Z = shp1d(ii_orig,1,Z, sh, n_elm, type);
                        
                        Si = shp1d_ii0Z;
                        dSi=shp1d_ii1Z;
                        
                        glf_addition(ii_1)=glf_addition(ii_1)+2*pi*((dwdr*Si + dwdr2*dSi)*R*s1R2-beta*P*dz*r*Si)*WI;
                        ...
                        if current_t>=op_time
                            if r_stent > r
                                ...
                                glf_addition(ii_1)=glf_addition(ii_1)+2*pi*F_penalty*WI;
                            end
                        end
                        glf_addition(ii_2)=glf_addition(ii_2)+2*pi*(dwdz2*dSi*R*s1R2 + beta*P*dr*r*Si)*WI;
                        ...
                        for j=1:npe
                            ...
                            shp1d_jj0Z = shp1d(jj,0,Z, sh, n_elm, type);
                            shp1d_jj1Z = shp1d(jj,1,Z, sh, n_elm, type);
                            
                            SiSj=shp1d_ii0Z*shp1d_jj0Z;
                            SidSj=shp1d_ii0Z*shp1d_jj1Z;
                            dSiSj=shp1d_ii1Z*shp1d_jj0Z;
                            dSidSj=shp1d_ii1Z*shp1d_jj1Z;
                            
                            
                            glk_addition(ii_1,jj_1)=glk_addition(ii_1,jj_1)+2*pi*((ddwddr*SiSj+ddwdrdr2*(SidSj+dSiSj)...
                                +ddwddr2*dSidSj)*R*s1R2-beta*P*dz*SiSj)*WI;
                            glk_addition(ii_1,jj_2)=glk_addition(ii_1,jj_2)+2*pi*((ddwdrdz2*SidSj+ ddwdr2dz2*dSidSj)*...
                            ...
                            if current_t>=op_time
                                if r_stent > r
                                    ...
                                    glk_addition(ii_1,jj_1)=glk_addition(ii_1,jj_1)+2*pi*K11_penalty*WI;
                                    glk_addition(ii_1,jj_2)=glk_addition(ii_1,jj_2)+2*pi*K12_penalty*WI;
                                end
                            end
                            glk_addition(ii_2,jj_2)=glk_addition(ii_2,jj_2)+2*pi*(ddwddz2*dSidSj*R*s1R2+beta*P*r*SidSj)*WI;
                            
                            glk_addition(ii_2,jj_1)=glk_addition(ii_2,jj_1)+2*pi*((ddwdrdz2*dSiSj+ddwdr2dz2*dSidSj)*...
                ...
                glk_segments{elm} = glk_addition;
                glf_segments{elm} = glf_addition;
                
                TD_L1_sliced(elm,:,:) = TD_L1_part;
                TD_L2_sliced(elm,:,:) = TD_L2_part;
            end
            
            timer.t4 = timer.t4 + (cputime - cput4);
            cput5 = cputime;
            
            % Merge parallel data back into the main matrix
            for source_elm = 1:n_elm
                glf_addition = glf_segments{source_elm};
                glk_addition = glk_segments{source_elm};
                for source_index = 1:size(glf_addition,1)
                    additional = (source_elm-1)*(npe-1)*2;
                    true_index = source_index + additional;
                    glf(true_index) = glf(true_index)+glf_addition(source_index);
                    glk(true_index,:) = glk(true_index,:)+glk_addition(source_index,:);
                end
            end
            
            timer.t5 = timer.t5 + (cputime - cput5);
            ...
            %             dx=glk\glf;
            dx = fem_solve(glk,glf);
            
            x_n=x_np-dx;
            % error
            itr_error = sqrt(sum(dx.^2)/sum(x_n.^2));
            
            timer.t6 = timer.t6 + (cputime - cput6);
        end
        ...
        %initialize the values at around center
        c_sum = 0;
        th_sum =0.0;
        r_sum=0.0;
        S1_sum=0.0;
        ...
        for elm=1:n_elm
            for k=1:n_Gauss_pt
                ...
                TD_L1_sliced(elm, k, nt+1) = L1;
                TD_L2_sliced(elm, k, nt+1) = L2;
                ...
                if nt+1 <= num_DL  %If there is still initial mass
                    ...
                    dwdL_k1= Mf_r(1)*G_h*dWcdx(Ln_k1, kc);
                    dwdL_k2= Mf_r(2)*G_h*dWcdx(Ln_k2, kc);
                    dwdL_k3= Mf_r(3)*G_h*dWcdx(Ln_k3, kc);
                    dwdL_k4= Mf_r(4)*G_h*dWcdx(Ln_k4, kc);
                ...
                dwdL_e1= Me*G_e(1)*dWedx(1,Ln_e1, Ln_e2, kc);
                dwdL_e2= Me*G_e(2)*dWedx(2,Ln_e1, Ln_e2, kc);
                ...
                for n_tau=n_tau0:nt+1
                    tau=(n_tau-1)*dt;
                    L1_tau=TD_L1_sliced(elm,k,n_tau);
                    L2_tau=TD_L2_sliced(elm,k,n_tau);
                    ...
                    dwdL_k1 = mc_tau(1) * (G_h/L1_tau)*dWcdx(Ln_k1, kc);
                    dwdL_k2 = mc_tau(2) * (G_h/L2_tau)*dWcdx(Ln_k2, kc);
                    dwdL_k3 = mc_tau(3) * (G_h/L_k3_tau)*dWcdx(Ln_k3, kc);
                    dwdL_k4 = mc_tau(4) * (G_h/L_k4_tau)*dWcdx(Ln_k4, kc);
                    ...
                    %Contribution of SM
                    Ln_m = G_m*L1/L1_tau;
                    dwdL1_m = dwdL1_m + mm_tau * (G_m/L1_tau)*dWmdx(Ln_m, kc)*w_dt;
                end
                ...
                if (more_it==max_more_it)&&(n_Zc-3<count && count < n_Zc+3)  %5 points around the middle
                    c_sum = c_sum+1;
                    th_sum =th_sum+thick_pt;
                    r_sum=r_sum+r;
                    S1_sum=S1_sum + S1;
                    S2_sum=S2_sum + S2;
                end
                ...
                if ( more_it==max_more_it && (mod(nt,print_step)== 1 || nt==init_dmg_t*n_dt-1))
                    if flag_LinearC > 0
                        ...
                        if nt+1 <= num_DL  %If there is still initial mass
                            ...
                            dW_k1dC_11 = (0.5/Ln_k1)*dWcdx(Ln_k1, kc) * G_h^2;
                            dW_k3dC_11 = (0.5/Ln_k3)*dWcdx(Ln_k3, kc) * G_h^2* sin(phi0)^2;
                            dW_k4dC_11 = (0.5/Ln_k4)*dWcdx(Ln_k4, kc) * G_h^2* sin(pi-phi0)^2;
                            
                            dW_mdC_11 = (0.5/Ln_m)*dWmdx(Ln_m, kc) * G_m^2;
                            
                            dW_k2dC_22 = (0.5/Ln_k2)*dWcdx(Ln_k2, kc) * G_h^2;
                            dW_k3dC_22 = (0.5/Ln_k3)*dWcdx(Ln_k3, kc) * G_h^2* cos(phi0)^2;
                            dW_k4dC_22 = (0.5/Ln_k4)*dWcdx(Ln_k4, kc) * G_h^2* cos(pi-phi0)^2;
                        ...
                        dWdC_11 = dWdC_11+(rho/Mt)* Me*G_e(1)/(2.0*L1)*dWedx(1, Ln_e1, Ln_e2, kc);
                        dWdC_22 = dWdC_22+(rho/Mt)* Me*G_e(2)/(2.0*L2)*dWedx(2, Ln_e1, Ln_e2, kc);
                        ...
                        for n_tau=n_tau0:nt+1
                            ...
                            dW_k1dC_11 = (0.5/Ln_k1)*dWcdx(Ln_k1, kc) * (G_h/L1_tau)^2;
                            dW_k3dC_11 = (0.5/Ln_k3)*dWcdx(Ln_k3, kc) * (G_h/L_k3_tau)^2* sin(phi0)^2;
                            dW_k4dC_11 = (0.5/Ln_k4)*dWcdx(Ln_k4, kc) * (G_h/L_k4_tau)^2* sin(pi-phi0)^2;
                            
                            dW_mdC_11  = (0.5/Ln_m)*dWmdx(Ln_m, kc) * (G_m/L1_tau)^2;
                            
                            dW_k2dC_22 = (0.5/Ln_k2)*dWcdx(Ln_k2, kc) * (G_h/L2_tau)^2;
                            dW_k3dC_22 = (0.5/Ln_k3)*dWcdx(Ln_k3, kc) * (G_h/L_k3_tau)^2* cos(phi)^2;
                            dW_k4dC_22 = (0.5/Ln_k4)*dWcdx(Ln_k4, kc) * (G_h/L_k4_tau)^2* cos(pi-phi)^2;
        ...
        timer.t7 = timer.t7 + (cputime - cput7);
    ...
    cput8 = cputime;
    %average the values at the middle point and write into the file
    if (more_it==max_more_it)
        ...
        if WRITE_FILE
            fprintf(fid0,'%10.1f\t%10.9f\t%12.10f\t%10.5f\t%10.5f\n', current_t, r_Zc, th_Zc, S1_Zc, S2_Zc);
        end
    end
    ...
    if WRITE_FILE
        for index = 1:size(r_nod,1)
            if index == size(r_nod,1)
                fprintf(shapefiler_fid, '%20.10f\n', r_nod(index));
                fprintf(shapefilez_fid, '%20.10f\n', z_nod(index));
            else
                fprintf(shapefiler_fid, '%20.10f\t', r_nod(index));
                fprintf(shapefilez_fid, '%20.10f\t', z_nod(index));
            end
        end
    end
    %     end
    timer.t8 = timer.t8 + (cputime - cput8);
...
if WRITE_FILE
    fclose(fid0);
    fclose(shapefiler_fid);
    fclose(shapefilez_fid);
end
%clear *
